    ...
    <div className="space-y-8">
      <section className="rounded-2xl border border-white/10 bg-slate-950/80 p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-[0.65rem] uppercase tracking-[0.25em] text-teal-300">
          ...
          <div className="flex items-center gap-3 text-xs">
            ...
            {isSyncing && (
              <span className="rounded-full border border-teal-300/50 bg-teal-300/10 px-3 py-1 text-teal-100">
                Syncing...
              </span>
            )}
          </div>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          {!isSupabaseConfigured ? (
            ...
            <div className="rounded-xl border border-white/10 bg-[#060815]/40 p-4 text-sm text-slate-200">
              <label className="block text-xs text-slate-500" htmlFor="session-email">
                Email address
              </label>
              <input
                id="session-email"
          ...
          <div className="rounded-xl border border-white/10 bg-[#060815]/40 p-4 text-xs text-slate-400">
            <p className="font-semibold text-slate-200">Session tips</p>
      ...
      <section className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-slate-950/80 p-5">
        <div className="flex items-center gap-3">
          {previous ? (
            <Link
              href={sessionHref(previous)}
              className="rounded-lg border border-white/10 px-4 py-2 text-sm text-slate-200 hover:bg-slate-900"
            >
              Previous: {previous.session}
            </Link>
          ) : (
            <span className="text-xs text-slate-500">First session</span>
          )}
          {next ? (
            <Link
            ...
            <span className="text-xs text-slate-500">Final session</span>
          )}
        </div>
